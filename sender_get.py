...
def list_labels(service):
    """Lists all the labels in the user's Gmail account."""
...
def create_table():
    """Creates the email_senders table if it doesn't exist."""
...
def insert_or_update_sender(sender_name, sender_email, folder_label, email_count):
    ...
    cursor.execute('''
        SELECT id, email_count FROM email_senders 
    ...
    if result:
        ...
        cursor.execute('''
            INSERT INTO email_senders (sender_name, sender_email, folder_label, email_count)
            VALUES (?, ?, ?, ?)
